...
class ProductosModel extends Query{
    ...
    $id_medida, $id_categoria, $foto, $catalogo)
    {
        $sql = "INSERT INTO productos (codigo, descripcion, precio_venta2, precio_venta, id_medida, id_categoria, foto, catalogo) VALUES (?,?,?,?,?,?,?,?)";
    ...
    {
        $sql = "UPDATE productos SET codigo=?, descripcion=?, precio_venta2=?, precio_venta=?, id_medida=?, id_categoria=?, foto=? WHERE id=?";
        $array = array($codigo, $nombre, $precio_venta2, $precio_venta, 
        $id_medida, $id_categoria, $foto, $id,);
        return $this->save($sql, $array);
    }
    ...
    {
        $sql = "SELECT id, descripcion, cantidad, precio_venta2, precio_venta, catalogo FROM productos WHERE descripcion LIKE '%".$valor."%' AND estado = 1 LIMIT 10";
        return $this->selectAll($sql);
    }
