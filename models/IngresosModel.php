...
class IngresosModel extends Query{
    ...
    public function registrarTraslado($productos, $fecha_create, $fecha_traslado, $total, $idCliente, $tipoTraslado, $id_usuario)
    {
        $sql = "INSERT INTO traslados (productos, fecha_create, fecha_traslado, total, id_bodega, traslado, id_usuario) VALUES (?,?,?,?,?,?,?)";
        $array = array($productos, $fecha_create, $fecha_traslado, $total, $idCliente, $tipoTraslado, $id_usuario);
    ...
    {
        $sql = "SELECT ap.*, cl.nombre, cl.telefono, cl.direccion FROM traslados ap INNER JOIN bodegas cl ON ap.id_bodega = cl.id WHERE ap.id = $idApartado";
        return $this->select($sql);
    }

    public function getCotizaciones()
    {
        $sql = "SELECT ap.*, cl.nombre FROM traslados ap INNER JOIN bodegas cl ON ap.id_bodega = cl.id where ap.traslado='INGRESO'"; 
