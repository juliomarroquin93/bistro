...
class NotaCreditoModel extends Query{
	...
	public function buscarRegistroVenta($idRegistro){
		...
		$anio = date("Y");
		$sql = "select v.id_cliente,v.uuid,v.total,c.nombre from ventas v join clientes c on v.id_cliente=c.id where v.numeroControlDte = '$idRegistro' and v.fecha like '%".$anio."%'";
	...
	public function buscarRegistroDte($idRegistro){
	    date_default_timezone_set('America/El_Salvador');
		$anio = date("Y");
		$sql = "select dte from dtes where id = (select id from ventas where numeroControlDte = '$idRegistro' and fecha like '%".$anio."%')"; 
		 return $this->selectAll($sql);
	}
    ...
    public function registrarVenta($productos, $total, $fecha, $hora, $metodo, $descuento, $serie, $pago, $docuemi, $numdocu, $vende,  $forma, $forma2, $idCliente, $idusuario, $correlativo, $numeroControlDte, $uuid, $codPuntoVentaMH, $sello, $vExentas, $vIva, $vGravadas, $claseDoc)
    {
        $sql = "INSERT INTO ventas (productos, total, fecha, hora, metodo, descuento, serie, pago, docuemi, numdocu, vende, forma, forma2, id_cliente, id_usuario, correlativo, numeroControlDte, uuid, codPuntoVentaMH, selloRecepcion, vExentas, vIva, vGravadas, claseDoc) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        $array = array($productos, $total, $fecha, $hora, $metodo, $descuento, $serie, $pago, $docuemi, $numdocu, $vende,  $forma, $forma2, $idCliente, $idusuario, $correlativo, $numeroControlDte, $uuid, $codPuntoVentaMH, $sello, $vExentas, $vIva, $vGravadas, $claseDoc);
