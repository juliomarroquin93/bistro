...
<script>
  ...
  var motivoDescripcion = combo.options[combo.selectedIndex].text;
   var documentoR =  document.getElementById("documento").value;
    var numdocumento = 0;
 var numdocumento = "<?php if($dteStr["dteJson"]["identificacion"]["tipoDte"]=="01")  {echo ($dteStr["dteJson"]["receptor"]["numDocumento"]);}else{ echo $dteStr["dteJson"]["receptor"]["nit"];} ?>";
...
xhttp.onreadystatechange = function() {
  ...
  if (this.readyState == 4 && this.status == 200) {
	...
	alert(jsonResponse.descripcionMsg);
	 $('#load').modal('hide'); 
	         const url = base_url + 'notaCredito/anular/' + <?php echo $idFactura; ?>;
            ...
            http.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (res.type == 'success') {
					location.href="<?php echo BASE_URL . 'notaCredito'; ?>";	
                    }
                }
            }
