...
$sql = "select * from planPagosPrevios where id =(".$data['idPlan'].")";
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
		...
		$monto = $row["monto"];
		$seguro = $row["cuotaSeguro"] * $row["plazo"];
		$fecha = $row["fecha"];
...
$sumaAbonoCapital = $sumaAbonoCapital + $detalle['abonoCapital'];
...
$html .= "<tr><td  class='d' colspan=2 ><p class='b'><b>Totales<b></p></td><td  class='d'><p class='b'>$".$monto."</p></td><td  class='d'><p class='b'>$".(number_format($totalInteres,2,'.',''))."<p/></td><td  class='d'><p class='b'>$".(number_format($seguro,2,'.',''))."</p></td><td  class='d'><p class='b'>$".(number_format($montoTotalPlan,2,'.',''))."</p></td><td  class='d'><p class='b'></p></td></tr>";
