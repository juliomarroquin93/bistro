...
$sql = "select d.id_venta,p.id_credito from detalle_pagoCuota d join planPagos p on d.id_plan=p.id_plan where d.id_plan = ".$idPlan." and d.cuota =".$cuota.";";
...
$sql = "select * from dtes where id=(".$idVenta.")";
$result = $conn->query($sql);
                ...
                <div class="row mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group mb-2">
							...
							<input class="form-control" type="text" id="idVentaPlan" value="<?php echo $idVenta; ?>" hidden>
							<input class="form-control" type="text" id="cuota" value="<?php echo $data['cuota']; ?>" hidden>
							<input class="form-control" type="text" id="idVenta" value="<?php echo $idFactura; ?>" hidden>
							<input class="form-control" type="text" id="idCredito" value="<?php echo $idCredito; ?>" hidden>
...
<script>
  ...
  var motivoDescripcion = combo.options[combo.selectedIndex].text;
   var documentoR =  document.getElementById("documento").value;
    var numdocumento = 0;
 var numdocumento = "<?php if($dteStr["dteJson"]["identificacion"]["tipoDte"]=="01" || $dteStr["dteJson"]["identificacion"]["tipoDte"]=="11")  {echo ($dteStr["dteJson"]["receptor"]["numDocumento"]);}else{ echo $dteStr["dteJson"]["receptor"]["nit"];} ?>";
...
xhttp.onreadystatechange = function() {
  ...
  if (this.readyState == 4 && this.status == 200) {
	...
	alert(jsonResponse.descripcionMsg);
	 $('#load').modal('hide'); 	 
	 var indice = parseFloat(cuota.value);
	 planActual[indice].Pago = 'Pendiente';
	 const url = base_url + 'ventas/anularDtePagoCuota';
            ...
            http.send(JSON.stringify({
                idVenta: idVentaPlan.value,
				planPago : JSON.stringify(planActual),
				idPlan : idPlan.value,
