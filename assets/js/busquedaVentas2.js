...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (e.keyCode === 13) {
        agregarProducto(inputBuscarGasto.value,0,0,0,0,'Normal');
  }
        return;
    })
    ...
    $("#buscarProductoNombre").autocomplete({
        ...
        select: function (event, ui) {
            ...
            agregarProducto(ui.item.label, ui.item.id, 1, ui.item.stock, precio, ui.item.catalogo);
            inputBuscarNombre.value = '';
            inputBuscarNombre.focus();
            return false;
        }
    ...
    http.onreadystatechange = function () {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (res.estado) {
                ...
                agregarProducto(res.datos.descripcion, res.datos.id, 1, res.datos.cantidad, precio,res.datos.catalogo);                
            }else{
...
function agregarProducto(descripcion, idProducto, cantidad, stockActual, precio, catalogo) {
    if (localStorage.getItem(nombreKey) == null) {
        listaCarrito = [];
    } else {
...
if (nombreKey === 'posVenta2'){
		...
		catalogo : chekServicio.checked || idProducto==0 ? "Servicio" :  chekExento.checked ? "Exento" :"Normal"
