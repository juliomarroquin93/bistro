    ...
    public function registrarCompra()
    {
        ...
        if (!empty($datos['productos'])) {
            ...
            if (empty($idproveedor)) {
                ...
                $res = array('msg' => 'LA FORMA ES REQUERIDA', 'type' => 'warning');
            } else {
                ...
                if ($saldo['saldo'] >= $total) {
                    ...
                    $compra = $this->model->registrarCompra($datosProductos, $total, $fecha, $hora, $serie, $forma, $idproveedor, $this->id_usuario);
                    if ($compra > 0) {
                        foreach ($datos['productos'] as $producto) {
                            $result = $this->model->getProducto($producto['id']);
                            //actualizar stock
