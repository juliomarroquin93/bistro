...
class Cajas extends Controller
{
    ...
    public function getDatos()
    {
        ...
        $consultaVentas2 = $this->model->getCompras($this->id_usuario);
        $ventas2 = ($consultaVentas2['total'] != null) ? $consultaVentas2['total'] : 0;
        ...
        $data['egresos'] = number_format($ventas2 + $gastos, 2, '.', '');
    ...
    public function cerrarCaja()
    {
        ...
        if ($result == 1) {
            $this->model->actualizarApertura('ventas2', $this->id_usuario);
    ...
    public function historialRepote($idCaja)
    {
        ...
        $data['movimientos']['saldoDecimal'] = number_format($datos['monto_final'] + $datos['monto_inicial'] - $datos['gastos'], 2);
        $this->views->getView('cajas', 'reporte', $data);
        $html = ob_get_clean();
        $dompdf = new Dompdf();
        $options = $dompdf->getOptions();
